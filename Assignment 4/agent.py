...
class Agent:
    ...
    def __init__(self, env, env_name, device, policy, theta, custom_reward=None, running_state=None, num_threads=1):
        self.env = env
        self.policy = policy
        self.device = device
    ...
    def collect_samples(self, min_batch_size, mean_action=False, render=False):
        ...
        for i in range(self.num_threads-1):
            worker_args = (i+1, queue, self.env, self.env_name, self.policy, self.theta, self.custom_reward, mean_action,
                           False, self.running_state, thread_batch_size)
            workers.append(multiprocessing.Process(target=collect_samples, args=worker_args))
        for worker in workers:
            worker.start()

        memory, log = collect_samples(0, None, self.env, self.env_name, self.policy, self.theta, self.custom_reward, mean_action,
